...
Use targeted tests during implementation and reserve full regressions for integration/final acceptance. One Agent owns a slice end-to-end by default. Treat `superpowers:*` as opt-in: do not invoke it for ordinary, simple, narrowly scoped, or targeted work unless the user explicitly requests it. It is appropriate only for coarse-grained stage/slice work, cross-layer or high-risk changes, broad refactors, or similarly complex tasks, and only the smallest directly applicable set may be used. Do not search for or invoke a skill that is not installed; every used skill needs an explicit exit condition, and two unchanged failures trigger diagnosis rather than retry.

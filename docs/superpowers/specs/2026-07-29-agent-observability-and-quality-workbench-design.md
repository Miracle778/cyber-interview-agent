...
> 2026-07-31 成熟度修订：运行中心与 Trace 设计保持有效；质量实验室当前实现属于 v1“初版质检 / 历史结果复检”。面向最终业务结果的 Rule、适用性分级、最小 Judge 视图和真实 Agent 回归采用
> `2026-07-31-agent-evaluation-v2-design.md`，不能从本文件的目标态描述推断为已全部实现。
...
- 经真实案例校准的确定性业务规则未来可以形成质量门禁；当前 v1 只做评估证据完整性检查，主观 Judge 只提供证据和提示。
...
本节是最初目标态设计。v1 已完成冻结历史 Execution、Judge、人工反馈和历史评估比较，但没有重新运行候选版本业务 Agent；现有确定性检查只验证评估证据事件是否齐全。当前能力与 v2 目标的准确边界见 2026-07-31 规格。
...
- v1 评估证据完整性检查：验证所需 Trace 是否可用；v2 确定性业务规则按关键路径执行；
- 在线异步评估：不阻塞用户结果；
- 独立 Judge：风险触发或采样执行；
...
- v1 证据完整性检查覆盖支持评估的运行；v2 是否扩大确定性业务规则覆盖范围需按真实案例校准；
...
- 目标态中，经验证的 Schema、来源、Workspace 边界和 Tool policy 规则可阻断发布；v1 当前未接入该门禁；
...
首个 Slice 完成后，产品只能声称“全局运行可查看、单次执行可追溯”。第三个 Slice 完成后，只能声称“可以冻结历史结果、执行初版 Judge、记录人工反馈并重新质检历史结果”；在候选业务 Agent 能基于冻结案例重新运行前，不能声称已经验证 Agent 改版效果。外部 OTel/Langfuse、长期趋势和自定义 Eval Pack 均不属于首版承诺。

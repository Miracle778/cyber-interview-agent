...
- 用户选择的模型用于本次交互命令的结构化意图解析和面向用户的自然语言生成。
- 上下文压缩仍属于 runtime middleware，可使用系统配置的摘要角色模型；它的调用和 token 使用单独记录，不伪装成用户选择模型的回答。
...
意图分类产生的是内部结构化数据，不把 JSON token 展示给用户。此阶段 UI 显示“正在理解你的指令”。解析完成后的业务状态和结果仍通过 SSE 到达。只有面向用户的自然语言模型内容使用 `assistant.delta`，不得把完整文本人工切片伪装成模型流。

...
- 微信、飞书等外部对话 Channel 负责碎片化复习和快速记录。
...
- **R7-R8 产品化与 Channel**：根据真实使用数据增加缓存、跨 Agent 指标、外部平台消息预算和降级策略。
...
目标：建立可由用户控制、可追溯并能被后续求职目标安全消费的个人资料底座。
...
- 简历助手只负责资料解释、来源定位、冲突发现、经历整理和待确认修改，不承担无目标岗位的泛化训练。
- 提供有界的 confirmed-profile application contract，供同一 Workspace 中用户明确选择的求职目标直接读取；不要求先发布到知识库。
- 通用个人资料发布、导出或跨模块共享作为后续可选能力，不阻塞岗位分析、项目训练、复盘和模拟面试。
...
- 已确认个人画像可通过受限查询供后续求职目标引用，pending/rejected Claim 和未获准敏感字段不会进入下游上下文。

## R4：求职目标与项目深挖训练

目标：以求职目标为一级索引，将 JD、可信个人经历、项目深挖、复习题和准备状态组织成第一条个性化面试训练闭环。

需求：

- 创建求职目标；首版允许只有岗位方向，也允许粘贴具体 JD，不要求招聘网站同步。
- Agent 提取岗位职责、技能、经验和项目要求，用户确认后才成为该目标的正式要求清单。
- 将每项要求与已确认的 Profile Claim、项目讲解卡或明确未知状态关联；以“已有证据 / 待深挖 / 资料不足 / 经历缺失”表达，不把单一匹配百分比作为核心结论。
- 用户选择一个真实项目启动连续深挖；通用讲解骨架覆盖背景、职责、方案选择、难点、结果、取舍与复盘，并叠加 JD 专项追问。
- 训练结束生成可复用的项目讲解卡；新增个人事实只生成待确认 Profile Proposal，不能自动更新画像。
- 将缺口分为资料、表达、知识和经历四类：分别进入资料补充、继续训练、题库候选或岗位风险，不允许 Agent 编造经历。
- 求职目标展示岗位要求覆盖、已准备项目、高风险追问和技术复习的可行动状态。
- 岗位详情后续聚合复习、复盘和模拟面试；第一切片不实现自动投递、招聘网站同步、完整申请进度自动化或正式 Todo Service。
- 岗位 Agent 使用统一 Session/Execution/Checkpoint、受限 Profile/Question Tool 和现有取消、恢复、审计边界。

验收：

- 用户能从一个岗位方向或 JD 创建目标，确认要求，并看到每项结论对应的 JD 原文和个人资料证据。
- 用户能选择一个项目完成连续深挖，得到项目讲解卡、分类缺口和更新后的准备状态。
- 知识缺口可以生成待确认复习题；资料补充必须经用户确认后才进入个人画像；经历缺口不会被模型伪造成项目事实。
- 删除求职目标不会删除已确认的通用个人资料和项目讲解事实。
- 目标工作区成为岗位分析、项目训练、复习、复盘和模拟面试的唯一组织中心。
...
- 分析回答表现，并沿用资料、表达、知识和经历四类缺口。
- 提炼个人项目经验并生成待确认的 Profile/项目讲解卡补充，不直接改写长期事实。
...
- 一场面试记录可以形成结构化复盘。
- 推断问题和原始转写有明确区分。
- 知识缺口能进入后续复习，项目经验能进入个人资料候选区，岗位准备状态能反映本次真实面试暴露的风险。
...
- 载入求职目标、已确认岗位要求、个人资料、项目讲解卡、题库和历史薄弱点。
- 选择技术面、HR 面和面试官风格。
- 选择固定题量，或由用户主动结束。
...
- 模拟上下文不会污染岗位主会话。
- 完整模拟面试消费既有项目讲解卡和高风险问题，不在模拟过程中自动改写个人资料。
...
## R8：微信、飞书原生对话 Channel

目标：让用户直接在微信、飞书等平台的原生聊天窗口中与本项目已有 Agent 对话，
完成碎片化复习、复盘记录和模拟面试；R8 不是移动浏览器适配，也不新建一套 Agent
Runtime。

需求：

- 为微信、飞书分别实现窄 Channel Adapter，把 webhook/event 转换为统一的可信
  `ChannelMessage`，并把 Agent 文本、追问、进度、审批和产物通知转换回平台消息或卡片。
- 建立外部平台账号、聊天会话与内部用户、workspace、Agent session 的服务端绑定；
  workspace/session/scope 不接受聊天消息或模型自报，首次绑定需要显式授权和可撤销凭证。
- 复用同一 application service、Agent session、LangGraph checkpoint、HITL、工具权限、
  usage 和知识发布流程；Channel 只负责 transport、身份映射和平台能力降级。
- 支持在聊天窗口选择或切换复习、复盘和模拟面试 Agent，继续已有会话或显式创建新会话；
  Web 与 Channel 看到同一份 session、进度、掌握度、pending action 和产物状态。
- 支持每日轻量复习、快速答题、必要追问、查看薄弱点与下一步建议，以及把面试后快速
  记录归入明确岗位；不支持的平台交互必须退化为清晰文本指令，不能静默丢失操作。
- 平台事件使用稳定 event ID、conversation ID 和 idempotency key；处理重复投递、乱序、
  超时、重试、进程重启和长 Agent 执行，确保一条用户消息不会重复推进 Graph。
- HITL 使用平台按钮/卡片或带版本的文本命令映射到现有 pending action；过期、重复或
  冲突决定返回明确状态，不允许 Channel 直接写 Vault 或伪造审批结果。
- 文件、语音转写和富文本先形成受限入站资源，经大小、类型、workspace、来源和恶意内容
  检查后再交给领域服务；Channel secret、签名、用户消息正文和文件内容默认不进入 trace。
- Channel 产生的内容同步回同一套会话、题库、报告和知识库审核流程，并支持用户解除绑定、
  停止主动消息和清理 Channel 凭证。

验收：

- 用户能在微信和飞书原生聊天窗口分别绑定身份、选择复习 Agent、完成一组短时答题，
  并在 Web 中继续同一内部 session；这项证据不能用移动浏览器页面替代。
- 用户能从 Channel 创建复盘快速记录，或进入已有模拟面试 session，消息与 Web 端状态一致。
- 重复 webhook、重试、乱序、后端重启和平台超时不会造成重复回答、重复 Graph 推进或重复发布。
- Channel 中的审批与 Web 使用同一 pending action/version/idempotency 规则；未授权账号、伪造
  workspace/session、越权文件和过期交互均被拒绝并留下安全审计。
- 微信或飞书暂时不可用时，内部 session/checkpoint 和知识产物保持一致，恢复后只补发安全的
  最终状态；Channel 不绕过 HITL、工具权限、路径策略和知识发布规则。
- R8 独立 spec 必须明确各平台实际接入形态、账号/租户限制、主动消息策略和本机服务安全暴露
  方案；不得默认把只监听 `127.0.0.1` 的开发后端直接暴露到公网。

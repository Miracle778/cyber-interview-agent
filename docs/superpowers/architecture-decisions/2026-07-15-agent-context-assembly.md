...
### 方案 D：单次 ReAct 解释并执行用户命令

让题库整理 Agent 在同一个模型 loop 中理解用户输入、查询候选题，并直接调用 inspect、publish、reject、regenerate 等工具完成操作。

优点：自然语言灵活，复杂命令可以由模型动态选择工具和执行顺序；对于需要多步探索、只读检索和根据中间结果继续推理的任务，ReAct 能减少固定编排。

拒绝其作为题库命令入口的原因：

- “发布第 6 题”等明确命令也必须产生模型调用，增加 token、延迟和失败面；
- 多个工具依次改变候选状态后，后续“其他题”的集合可能不再对应用户输入时的同一 summary version；
- 重写成功而发布失败会形成部分成功，用户或基础设施重试可能重复副作用；
- HTTP、模型和工具三层重试叠加后，幂等键、receipt 和恢复点难以保持单一所有权；
- 模型同时决定选择范围、工具顺序、调用次数和停止条件，扩大了高风险副作用的非确定性边界；
- ReAct 所需的工具 schema、调用结果和候选正文会进一步扩大上下文。

ReAct 仍适用于真正的探索型能力，例如从多份来源按需读取片段、选择只读知识工具、根据结果继续补充题目或进行深入讨论。即使未来统一对话主 Agent 通过 ReAct 路由到题库能力，它也只能调用窄的 curation command application service，不能绕过本决定的计划、校验、幂等和领域副作用边界。
...
命令链路固定采用 `Plan -> Validate -> Execute`：模型最多生成计划，application service 在冻结的 summary version 上解析稳定 candidate IDs、校验状态与权限、确定执行顺序并写入幂等 receipt。模型不得在同一次自由 ReAct loop 中直接完成多个领域副作用。

...
export function AgentProcessCard({ status, title, summary, children }: AgentProcessCardProps) {
  const Icon = status === "running" ? LoaderCircle : status === "waiting" ? Clock3 : status === "failed" ? TriangleAlert : CheckCircle2;
  ...
  if (status === "running" || status === "waiting" || status === "failed") {
    return <section className={`agent-process-card is-${status}`} role={status === "failed" ? "alert" : "status"}><header><Icon size={17} /><strong>{title}</strong></header>{body}</section>;
  }

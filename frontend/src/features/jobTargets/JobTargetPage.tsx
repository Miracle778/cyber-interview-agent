...
import {
  ...
  getCurrentAnalysis, getCurrentDeepDive, getReadiness, getRetrospectiveSummary, listJobTargets, listRequirements,
...
export function JobTargetPage({ workspace }: { workspace: WorkspaceConfig | null }) {
  ...
  useEffect(() => {
    ...
    if (returnTab === "deep-dive" || returnTab === "review") { setProjectPickerOpen(false); setTab(returnTab); }
  ...
  const retrospectiveSummary = useQuery({ queryKey: ["job-targets", workspaceId, target?.id, "retrospective-summary"], queryFn: ({ signal }) => getRetrospectiveSummary(workspaceId, target!.id, signal), enabled: Boolean(target) });
  ...
  const profileSummary = profile.data ? {
    confirmedItems: profile.data.experiences.length + profile.data.projects.length + profile.data.skills.length + profile.data.education.length + profile.data.certifications.length + profile.data.achievements.length,
    projectCount: profile.data.projects.length,
  } : undefined;
  ...
  return <section className={`job-target-page${targetListCollapsed ? " job-target-page--list-collapsed" : ""}`}><JobTargetList targets={targets} selectedId={target?.id} collapsed={targetListCollapsed} onToggleCollapsed={() => setTargetListCollapsed((value) => !value)} onSelect={(id) => { setSelectedId(id); setProjectPickerOpen(false); setTab("overview"); }} onCreate={() => setCreateOpen(true)} />
    <main>{target ? <JobTargetWorkspace target={target} analysis={analysis.data} tab={tab} onTab={setTab} onCompleteInfo={() => setTargetInfoOpen(true)}>
      {tab === "overview" ? <JobTargetOverview target={target} readiness={readiness.data} analysis={analysis.data} retrospectiveSummary={retrospectiveSummary.data} profileSummary={profileSummary} onEditJd={() => setJdOpen(true)} onCompleteInfo={() => setTargetInfoOpen(true)} onStartAnalysis={() => runAnalysis.mutate()} onControl={(action) => analysisControl.mutate(action)} onNavigate={setTab} onStartTargetReview={() => navigate(`/review?${new URLSearchParams({ create: "1", scope: "job-target", jobTargetId: target.id, scopeLabel: target.roleName || "当前求职目标", returnTo: `/targets?${new URLSearchParams({ target: target.id, tab: "overview" })}`, returnLabel: "返回求职目标" })}`)} onOpenRetrospectives={() => navigate(`/retrospectives?jobTargetId=${encodeURIComponent(target.id)}`)} /> : null}
      ...
      {tab === "review" ? <ProjectQuestionCandidates projectTitle={dive.data?.projectTitle} candidates={dive.data?.questionCandidates ?? []} busy={questionDecision.isPending || questionBatchDecision.isPending || questionEdit.isPending} onStartReview={activeProjectId && dive.data?.questionCandidates.some((item) => item.status === "confirmed") ? () => navigate(`/review?${new URLSearchParams({ create: "1", scope: "project", projectClaimId: activeProjectId, scopeLabel: dive.data?.projectTitle || "当前项目", returnTo: `/targets?${new URLSearchParams({ target: target.id, project: activeProjectId, tab: "review" })}`, returnLabel: "返回项目题" })}`) : undefined} onDecide={(id, decision) => questionDecision.mutate({ id, decision })} onBatchDecide={(ids, decision) => questionBatchDecision.mutate({ ids, decision })} onEdit={(id, title, question) => questionEdit.mutate({ id, title, question })} /> : null}

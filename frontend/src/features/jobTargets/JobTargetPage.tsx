...
export function JobTargetPage({ workspace }: { workspace: WorkspaceConfig | null }) {
  ...
  const jd = useMutation({ mutationFn: async (body: string) => { const version = await createDocumentVersion(workspaceId, target!.id, body); return confirmDocumentVersion(workspaceId, target!, version.id); }, onSuccess: async () => { setJdOpen(false); await refresh(); } });

...
import type { AnalysisRun, AnalysisWorkItem } from "./retrospectiveTypes";

const ACTIVE = new Set(["queued", "running", "stopping"]);

function stageLabel(run: AnalysisRun, items: AnalysisWorkItem[]) {
  if (run.status === "completed") return "复盘分析已完成";
  if (run.status === "failed") return "分析中断，已有结果仍然保留";
  if (run.status === "stopped") return "分析已停止，随时可以继续";
  if (run.currentWorkKey === "question_reduce") return "正在合并各段识别结果";
  if (run.stage === "question_extraction") {
    const windows = items.filter((item) => item.workKey.startsWith("question_extraction:") && item.status !== "skipped");
    const completed = windows.filter((item) => item.status === "completed").length;
    return windows.length > 1 ? `正在分段识别面试问题（${completed} / ${windows.length}）` : "正在识别面试问题";
  }
...
export function AnalysisProgress({ run, items = [], busy, onStop, onResume, onRetry }: {
  run: AnalysisRun;
  ...
  return <section className="analysis-progress" data-status={run.status} data-compact={completed || undefined} aria-live="polite">
    <div className="analysis-progress__status">
      <span><StatusIcon size={20} aria-hidden="true" /></span>
      <div><strong>{stageLabel(run, items)}</strong><small>{completed ? `${run.completedItems} 个分析步骤已保存` : active && run.completedItems > 0 ? "分析仍在继续，已完成的问题可以先看" : "刷新或离开不会丢失已完成结果"}</small></div>
    </div>
    {!completed ? <div className="analysis-progress__meter">

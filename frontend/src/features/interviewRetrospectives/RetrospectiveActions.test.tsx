...
describe("RetrospectiveActions", () => {
  ...
  it("offers selected safe sections without a raw transcript option", () => {
    ...
    expect(screen.getByText(/发布内容不会包含原始转写/)).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: "生成 Knowledge 草稿" }));
  ...
  it("links a generated draft back to Knowledge", () => {
    ...
    expect(screen.getByRole("link", { name: "前往 Knowledge 查看" })).toHaveAttribute("href", "/knowledge");
  });
});

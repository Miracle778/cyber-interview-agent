...
import { Archive, Eraser, MoreHorizontal, RotateCcw, Trash2, TriangleAlert } from "lucide-react";
...
export function RetrospectiveLifecycleActions({ retrospective, onChanged, onError }: {
  ...
  return <>
    <div className="retrospective-lifecycle-actions" aria-label="复盘管理">
      <details className="retrospective-lifecycle-menu">
        <summary aria-label="更多复盘操作"><MoreHorizontal size={18} /> 更多操作</summary>
        <div>
          ...
          {retrospective.lifecycleStatus === "active" ? <Button size="sm" variant="ghost" disabled={busy} onClick={() => void run("archive")}><Archive size={15} />归档</Button> : null}
          ...
          {["archived", "recycled"].includes(retrospective.lifecycleStatus) ? <Button size="sm" variant="ghost" disabled={busy} onClick={() => void run("restore")}><RotateCcw size={15} />恢复</Button> : null}

import { AlertTriangle, ArrowLeft, Bot, CheckCircle2, FileText } from "lucide-react";
...
export function CurationArtifactDetail({ candidate, onClose }: { candidate: QuestionCandidate; onClose: () => void }) {
  const sourceSupported = candidate.answerBasis === "source"
    && candidate.materialSupport === "sufficient"
  ...
  return <aside className="curation-artifact-detail" aria-label="Markdown 文件详情">
    ...
    <div className="curation-artifact-detail__body">
      ...
      <section className={`curation-quality-detail${sourceSupported ? " is-supported" : " is-warning"}`} aria-label="候选质量">{sourceSupported ? <CheckCircle2 size={17} /> : <AlertTriangle size={17} />}<div><strong>{candidate.answerBasis === "source" ? "答案主要来自原资料" : candidate.answerBasis === "mixed" ? "答案含 AI 补全" : "答案主要由 AI 生成或依据不明"}</strong><p>{candidate.materialSupport === "sufficient" ? "材料支撑充分" : candidate.materialSupport === "partial" ? "材料只能部分支撑答案" : "材料支撑较少，需要人工核对"}{candidate.needsReview ? "；发布前必须人工复核。" : "。"}</p></div></section>
      ...
      {candidate.sourceAnswer || candidate.supplementalAnswer ? <section className="curation-artifact-advice" aria-label="答案来源说明"><FileText size={18} /><div><strong>答案来源说明</strong>{candidate.sourceAnswer ? <><p><b>原资料中的答案</b></p><p>{candidate.sourceAnswer}</p></> : null}{candidate.supplementalAnswer ? <><p><b>AI 补充</b></p><p>{candidate.supplementalAnswer}</p></> : null}</div></section> : null}

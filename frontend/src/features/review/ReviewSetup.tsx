...
export function ReviewSetup({ workspace, questions, onCreate, onCatalog, busy }: { workspace: WorkspaceConfig; questions: ActiveQuestion[]; onCreate: (request: CreateReviewRoundRequest) => void; onCatalog: () => void; busy: boolean }) {
  ...
  const reviewSources = useMemo(() => (sources.data ?? [])
    .filter((source) => questions.some((question) => question.sourceIds?.includes(source.id)))
    .map((source) => ({ ...source, questionCount: sourceQuestionCounts.get(source.id) ?? 0 })),
  [questions, sourceQuestionCounts, sources.data]);
  const available = questions.filter((item) => {
  ...
  const effectiveCount = mode === "source-file" ? Math.min(count, available) : count;
  ...
  return (
    <Card title="创建复习轮次" icon={<SlidersHorizontal size={18} />}>
      ...
      <div className="review-setup-summary"><span>匹配题目 {available} 道</span><span>本轮计划 {effectiveCount} 道</span>{mode === "source-file" && available > 0 && available < count ? <span>该资料不足 {count} 道，将按实际题量创建</span> : null}</div>
      <Button loading={busy} disabled={!effectiveModel || available < 1 || (mode !== "source-file" && available < count) || count < 1 || count > 50 || topicRequired || sourceRequired} onClick={() => onCreate({ workspaceId: workspace.id, selectedTopics: mode === "source-file" ? [] : selectedTopics, difficulties: [difficulty], mode, questionCount: effectiveCount, allowFollowUp: true, answerModelId: effectiveModel, reasoningEffort: reasoning, ...(mode === "source-file" ? { sourceId: selectedSourceId } : {}) })}><Play size={16} />开始复习</Button>
      ...
      {mode === "source-file" && !sources.isPending && reviewSources.length === 0 ? <section className="review-readiness review-readiness--low review-readiness--compact" role="status"><span className="review-readiness__icon"><TriangleAlert size={20} /></span><div><h3>还没有可按资料复习的题目</h3><p>请先整理资料并将候选题确认入库。</p></div><Button variant="secondary" onClick={onCatalog}><BookOpenCheck size={16} />去题库整理</Button></section> : null}
      {mode !== "source-file" && available < count ? <section className="review-readiness review-readiness--low review-readiness--compact" role="status"><span className="review-readiness__icon"><TriangleAlert size={20} /></span><div><h3>当前筛选题量不足</h3><p>可调整难度或题量，也可以先确认更多题目后再创建轮次。</p></div><Button variant="secondary" onClick={onCatalog}><BookOpenCheck size={16} />去题库整理</Button></section> : null}
    </Card>
  );
}

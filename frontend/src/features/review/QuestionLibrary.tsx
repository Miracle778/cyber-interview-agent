...
export function QuestionLibrary({ workspace, sources, initialCandidateId = null, onOpenSession }: { workspace: WorkspaceConfig; sources: KnowledgeSource[]; initialCandidateId?: string | null; onOpenSession: (sessionId: string) => void }) {
  ...
  const grouped = useMemo(() => {
    const result = new Map<string, Map<string, NonNullable<typeof candidates.data>>>();
    for (const candidate of candidates.data ?? []) {
      const topicName = candidate.question.topics[0] || "未分类";
      const byDifficulty = result.get(topicName) ?? new Map<string, NonNullable<typeof candidates.data>>();
      const items = byDifficulty.get(candidate.question.difficulty) ?? [];
      byDifficulty.set(candidate.question.difficulty, [...items, candidate]);
      result.set(topicName, byDifficulty);
    }
    return [...result.entries()].sort(([left], [right]) => left.localeCompare(right, "zh-CN"));
  }, [candidates.data]);

...
import type { ReviewEvaluationStage } from "./ReviewConversation";
import type { ReviewKeyPointCoverage, ReviewRound } from "./reviewTypes";
...
const evaluationStageLabel: Record<ReviewEvaluationStage, string> = {
  preparing: "理解本次回答",
  checking_key_points: "对照必答方向",
  deciding_follow_up: "生成反馈与下一步",
};
...
type PendingDirection = Pick<ReviewKeyPointCoverage, "point" | "status">;
...
function pendingDirections(
  ...
  return historicalMissingPoints.map((point) => ({ point, status: "uncovered" }));
}

export function ReviewRuntimePanel({ round, evaluationStage = "preparing" }: { round: ReviewRound | null; evaluationStage?: ReviewEvaluationStage }) {
  const [collapsed, setCollapsed] = useState(false);
  const [now, setNow] = useState(() => Date.now());
  const providers = useQuery({ queryKey: ["providers"], queryFn: listProviders });
  const latest = round?.attempts.at(-1);
  const evaluating = latest?.status === "evaluating";
  useEffect(() => {
    if (!evaluating) return;
    setNow(Date.now());
    const timer = globalThis.setInterval(() => setNow(Date.now()), 1000);
    return () => globalThis.clearInterval(timer);
  }, [evaluating, latest?.id]);
  const evaluation = latest?.evaluation;
  const mastery = latest?.masterySuggestion ?? evaluation?.mastery_suggestion ?? "unknown";
  const directionsToImprove = pendingDirections(
    latest?.coverage,
  ...
  const evaluationSeconds = latest?.evaluationStartedAt
    ? elapsedSeconds(latest.evaluationStartedAt, latest.evaluationCompletedAt ?? new Date(now).toISOString())
    : null;
  const runtimeStatus = evaluating
    ? evaluationStageLabel[evaluationStage]
    : round?.status === "waiting_for_input"
      ? "等待回答"
      : "已更新";
  return <aside className={`review-runtime-panel${collapsed ? " is-collapsed" : ""}`} aria-label="本题反馈">
    ...
    {!collapsed && round ? <div className="review-insight-content">
      ...
      <details className="curation-runtime-disclosure review-runtime-disclosure is-static" open><summary onClick={(event) => event.preventDefault()}><span><Activity size={16} />运行详情</span><small>{round.usage.callCount} 次调用</small></summary><div className="curation-runtime-disclosure__body"><dl><div><dt>执行状态</dt><dd>{runtimeStatus}</dd></div><div><dt>本次耗时</dt><dd>{evaluationSeconds === null ? "—" : formatElapsedSeconds(evaluationSeconds)}</dd></div><div><dt>评价模型</dt><dd title={modelLabel}>{modelLabel}</dd></div><div><dt>思考强度</dt><dd>{reasoningLabel[round.settings.reasoning_effort]}</dd></div><div><dt>Token</dt><dd>{formatTokens(round.usage.totalTokens)}</dd></div></dl><div className="curation-context-compact"><div className="curation-context-ring" style={{ "--context-progress": `${contextPercentage * 3.6}deg` } as CSSProperties}><span>{contextPercentage}%</span></div><div><small>当前上下文 / 压缩阈值</small><strong>{formatTokens(currentTokens)} / {thresholdTokens > 0 ? formatTokens(thresholdTokens) : "—"}</strong></div></div><p className="review-runtime-local-note">查看提示和答案直接读取本轮题库，不调用模型；提交回答后才会统计调用与 Token。</p></div></details>
      <details className="review-runtime-disclosure review-key-points" open><summary><span><BookOpenCheck size={17} />待完善关键点</span><small>{directionsToImprove.length} 项</small><ChevronDown size={16} /></summary><div>{directionsToImprove.length ? <ol>{directionsToImprove.map(({ point, status }, index) => <li key={`${index}:${point}`} title={point}><span>{index + 1}</span><p>{point}</p><em className={`is-${status}`}>{status === "partial" ? "部分覆盖" : "未覆盖"}</em></li>)}</ol> : <p className="status-note">评价完成后，这里会列出需要完善的知识点。</p>}</div></details>
    </div> : null}
  </aside>;
}

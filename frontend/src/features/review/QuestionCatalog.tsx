...
export function QuestionCatalog({ workspace }: { workspace: WorkspaceConfig }) {
  ...
  const startBulk = useMutation({ mutationFn: ({ sessionId, version, candidateIds, confirmedAiCandidateIds }: { sessionId: string; version: number; candidateIds: string[]; confirmedAiCandidateIds: string[] }) => startBulkPublication(sessionId, version, candidateIds, commandId(), confirmedAiCandidateIds), onMutate: () => setBulkPublishError(null), onSuccess: (accepted) => { setBulkPreflight(null); setBulkAiConfirmed(false); setBulkPublishError(null); setBulkOperation(null); setActiveInteraction({ kind: "bulk", executionId: accepted.executionId, operationId: accepted.operationId }); }, onError: (error) => setBulkPublishError(error instanceof Error ? error.message : "批量发布未开始，请刷新清单后重试。") });

...
import type { BulkPublication, BulkPublicationPreflight, CurationMessage, CurationSession } from "./reviewTypes";
...
export function QuestionCatalog({ workspace }: { workspace: WorkspaceConfig }) {
  ...
  useEffect(() => {
    const latest = agentEvents.events.at(-1);
    if (!latest || latest.id <= lastHandledEventId.current) return;
    lastHandledEventId.current = latest.id;
    if (["session.message.created", "curation.command.resolved", "publication.changed", "execution.completed", "execution.cancelled", "execution.failed", "execution.interrupted"].includes(latest.type)) {
      void refresh();
      if (latest.type === "publication.changed") void refreshCandidates();
    }
    if (activeInteraction?.kind === "bulk" && activeInteraction.operationId && ["execution.completed", "execution.cancelled", "execution.failed", "execution.interrupted"].includes(latest.type) && latest.executionId === activeInteraction.executionId) {

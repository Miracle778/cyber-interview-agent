  ...
  return (
    <section className="question-detail" aria-label="题目详情" key={candidate.id}>
      <div className="question-detail__scroll">
        ...
        {candidate.sourceAnswer || candidate.supplementalAnswer ? <section className="source-evidence" aria-label="答案来源说明"><div><FileText size={16} /><strong>答案来源说明</strong></div><div>{candidate.sourceAnswer ? <><strong>原资料中的答案</strong><p>{candidate.sourceAnswer}</p></> : null}{candidate.supplementalAnswer ? <><strong>AI 补充</strong><p>{candidate.supplementalAnswer}</p></> : null}</div></section> : null}

...
- 用户真实整理 Execution `d0bdb1a1-003c-4711-92f6-042afddf94ff` 在 discovery wave 失败。数据库证据显示 75 个 deterministic 与 2 个 model Work Item 已 completed，失败项为 unit 0；Trace 精确定位 GLM 返回的第 20 个 seed 缺少 `source_ref` 且含 null 引用，触发 `StructuredOutputValidationError`。新增 RED 复现后，在 `QuestionSeedChunk` 预校验中仅丢弃无有效证据引用的 raw seed；未知/跨来源引用继续硬失败。题目整理契约、Graph、Planner、Scheduler、Work Item、AgentFactory 受影响回归 `92 passed`。

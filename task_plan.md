...
| 11. Agent 上下文组装与命令解释收敛 | 设计已确认，实施计划已写，待执行 | 通用 token-budget ContextAssembler、持久领域焦点、确定性命令优先、一次性结构化 classifier |
...
1. 按 `2026-07-15-agent-context-assembly.md` 四个 Task 以 TDD 实施，先完成通用组装与持久焦点。
2. 迁移题库整理命令链路并删除固定 8 条上下文逻辑，完成重启与压缩集成测试。
3. 接回原 R2 浏览器验收、最终全量回归、verification/learning 与文档门禁。

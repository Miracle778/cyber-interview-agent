...
## 2026-08-05：后台 Agent 可见性、质量检查进度与模型批量设置（自动验证完成）

- [x] 将质量检查改为后台 `quality.evaluate` Execution，复用现有确定性检查与 AI Judge，并用同一 ID 关联 Execution、Trace 和质量报告；
- [x] 质量页按 Execution 与 Operation 状态展示“准备检查依据 / AI 检查业务结果 / 保存质量报告”，支持跳转运行中心；
- [x] 将 `quality.evaluate`、`profile.ingest`、`profile.assess` 注册为运行中心默认可见的 system Agent，同时保持公共 API 不可创建；
- [x] 左侧“Agent 运行中心”在当前工作区存在运行中任务时显示实时数字角标，桌面与移动导航共享同一状态；
- [x] 设置页支持选择一次模型后原子覆盖全部任务角色，并保留逐项调整；
- [x] 后端相关 53 项、前端全量 436 项、定向 7 项、Ruff、compileall、TypeScript、production build 与差异检查通过；
- [ ] 重启本地后端后完成 5175 真实数据浏览器验收；当前页面可达，但后端未运行，未伪造 Provider 或模型绑定写入。

## 2026-08-05：运行中心默认可见 system Agent 列表口径修复（实现完成）

- [x] Execution Summary 与 SSE 事件显式返回 `runCenterDefaultVisible`；
- [x] 运行中心不再无条件隐藏所有 system Agent，质量检查和画像整理按注册中心合同默认展示；
- [x] 未声明默认可见的内部 system Agent 继续保持隐藏，仅在显式包含系统 Agent 时展示；
- [x] 质量检查 Execution 可从运行中心列表直接进入独立 Trace 详情；
- [x] 后端相关 39 项、前端运行中心 16 项与 TypeScript 检查通过。

## 2026-08-05：题目整理漏题召回方案与首个 Gold Set（设计完成）

- [x] 用本地授权的 `Java.md` 复核现有 planner：112 个稳定 section 仅建立 4 个确定性题目锚点，确认“材料处理完成”不能证明“题目没有遗漏”；
- [x] 在渐进式题目整理正式设计中补充“确定性显式锚点 + 模型语义发现 + 独立 coverage audit + 单一 reducer”方案；
- [x] 收紧完成语义：发现或查漏工作未终结、达到上限但未审计时只能标记部分完成/需要查漏；
- [x] 在 Eval v2 中补充可靠分母、Gold hash 绑定、人工确认状态和召回/精确率/重复率指标；
- [x] 创建本地 `Java.md` 预标注 manifest，共 36 个待确认问题，不复制原文且不进入 Git；
- [ ] 用户人工确认预标注、拆分粒度和遗漏项后，将 manifest 冻结为首个正式 Gold Set；
- [x] 实现独立 coverage audit：全文窗口与已有题目索引重新比对，漏题种子进入统一 Seed reducer；查漏耗尽重试时禁止假完成；运行进度显示独立“查漏题目”阶段。
- [x] 实现 Gold 对齐器：一对一来源锚点匹配，输出总召回、显式/隐式/关键题召回、漏题 ID、重复候选和无锚点候选；未确认 Gold 默认拒绝形成正式分数。
- [x] 用 Java 预标注草稿测得新显式锚点基线 19 / 36（临时总召回 52.78%、显式 95%、关键题 75%、隐式 0%）；该数仅用于确认查漏必要性，不是正式质量结论。
- [ ] 用户人工确认预标注后，运行完整 Provider 方案并冻结“旧基线 / 显式锚点 / 完整查漏”三组正式结果。

...
| 11. Agent 上下文组装与命令解释收敛 | 实现与自动验证完成，浏览器待验收 | 通用 token-budget ContextAssembler、持久领域焦点、确定性命令优先、一次性结构化 classifier；跨模块定向回归 70 passed |
...
1. 对阶段 11 运行一次最终后端/前端全量回归与 build。
2. 完成持久焦点、压缩事实和无 Langfuse 依赖的浏览器验收。
3. 接回阶段 8–10 的浏览器验收、verification/learning 与文档门禁。
